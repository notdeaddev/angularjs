...
describe('ngAnimate $$animateCssDriver', function () {
  ...
  describe('when active', function () {
    ...
    it(
      'should register the $$animateCssDriver into the list of drivers found in $animateProvider',
    ...
    describe('anchored animations', function () {
      ...
      they(
        'should return a runner with a $prop() method which will end the animation',
        ['end', 'cancel'],
        function (method) {
          var closeAnimation;
          module(function ($provide) {
      ...
      it('should end the animation for each of the from and to elements as well as all the anchors', function () {
        ...
        inject(function () {
          ...
          var animator = driver({
            from: fromAnimation,
            to: toAnimation,
            anchors: [anchorAnimation, anchorAnimation, anchorAnimation]
          });
      ...
      it('should create a clone of the starting element and place it at the end of the $rootElement container', inject(function (
        $rootElement
      ) {
      ...
      it("should then do an addClass('ng-anchor-in') animation on the cloned anchor and remove the old class", inject(function (
        $rootElement
      ) {
        var fromAnchor = jqLite('<div></div>');
        from.append(fromAnchor);
      ...
      they(
        'should only fire the ng-anchor-$prop animation if only a $prop animation is defined',
        ['out', 'in'],
        function (direction) {
      ...
      it('should provide an explicit delay setting in the options provided to $animateCss for anchor animations', inject(function (
        $rootElement
      ) {
        var fromAnchor = jqLite('<div></div>');
        from.append(fromAnchor);
      ...
      it('should begin the anchor animation by seeding the from styles based on where the from anchor element is positioned', inject(function (
        $rootElement
      ) {
      ...
      it("should then do an removeClass('out') + addClass('in') animation on the cloned anchor", inject(function (
        $rootElement
      ) {
        var fromAnchor = jqLite('<div></div>');
        from.append(fromAnchor);
      ...
      it('should add the `ng-anchor` class to the cloned anchor element', inject(function ($rootElement) {
        var fromAnchor = jqLite('<div></div>');
        from.append(fromAnchor);
        var toAnchor = jqLite('<div></div>');
        to.append(toAnchor);
      ...
      it('should add and remove the `ng-animate-shim` class on the in anchor element during the animation', inject(function (
        $rootElement
      ) {
        var fromAnchor = jqLite('<div></div>');
        from.append(fromAnchor);
      ...
      it('should add and remove the `ng-animate-shim` class on the out anchor element during the animation', inject(function (
        $rootElement
      ) {
        var fromAnchor = jqLite('<div></div>');
        from.append(fromAnchor);
      ...
      it('should create the cloned anchor with all of the classes from the from anchor element', inject(function (
        $rootElement
      ) {
      ...
      it('should remove the classes of the starting anchor from the cloned anchor node during the in animation and also add the classes of the destination anchor within the same animation', inject(function (
        $rootElement
      ) {
        var fromAnchor = jqLite('<div class="yes no maybe"></div>');
        from.append(fromAnchor);
      ...
      it('should not attempt to add/remove any classes that contain a `ng-` prefix', inject(function ($rootElement) {
      ...
      it('should not remove any shared CSS classes between the starting and destination anchor element during the in animation', inject(function (
        $rootElement
      ) {
      ...
      it('should continue the anchor animation by seeding the to styles based on where the final anchor element will be positioned', inject(function (
        $rootElement
      ) {
      ...
      it("should remove the cloned anchor node from the DOM once the 'in' animation is complete", inject(function (
        $rootElement
      ) {
        var fromAnchor = jqLite('<div class="blue green red"></div>');
        from.append(fromAnchor);
      ...
      it('should pass the provided domOperation into $animateCss to be run right after the element is animated if a leave animation is present', inject(function (
        $rootElement
      ) {
        toAnimation.structural = true;
      ...
      it('should fire the returned runner promise when the from, to and anchor animations are all complete', inject(function (
        $rootElement,
        $rootScope,
        $animate
      ) {
