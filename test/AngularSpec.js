...
describe('angular', function () {
  ...
  describe('copy', function () {
    ...
    they(
      'should copy source and ignore max depth when maxDepth = $prop',
  ...
  describe('csp', function () {
    ...
    it('should return true for noUnsafeEval if eval causes a CSP security policy error', function () {
      window.Function.and.callFake(function () {
  ...
  describe('parseKeyValue', function () {
    it('should parse a string into key-value pairs', function () {
      ...
      expect(parseKeyValue('escaped%20key=escaped%20value')).toEqual({ 'escaped key': 'escaped value' });
      expect(parseKeyValue('emptyKey=')).toEqual({ emptyKey: '' });
      expect(parseKeyValue('flag1&key=value&flag2')).toEqual({ flag1: true, key: 'value', flag2: true });
    });
    ...
    it('should parse a string into key-value pairs with duplicates grouped in an array', function () {
      ...
      expect(parseKeyValue('flag1&key=value&flag1')).toEqual({ flag1: [true, true], key: 'value' });
      expect(parseKeyValue('flag1&flag1=value&flag1=value2&flag1')).toEqual({ flag1: [true, 'value', 'value2', true] });
    });
  ...
  describe('toKeyValue', function () {
    it('should serialize key-value pairs into string', function () {
      ...
      expect(toKeyValue({ 'escaped key': 'escaped value' })).toEqual('escaped%20key=escaped%20value');
    ...
    it('should serialize duplicates into duplicate param strings', function () {
      ...
      expect(toKeyValue({ key: [323, 'value', true, 1234] })).toEqual('key=323&key=value&key&key=1234');
    });
  });
  ...
  describe('forEach', function () {
    ...
    it('should handle JQLite and jQuery objects like arrays', function () {
      ...
      forEach(jqObject.children(), function (value, key) {
        log.push(key + ':' + value.innerHTML);
      });
      expect(log).toEqual([]);
    });
    ...
    if (!msie) {
      describe('auto bootstrap restrictions', function () {
        ...
        it('should not bootstrap from an extension into a non-extension document, via SVG script', function () {
          ...
          expect(allowAutoBootstrap(createFakeDoc({ src: 'http://something', href: 'resource://something' }))).toBe(
            false
          );
          expect(
            allowAutoBootstrap(createFakeDoc({ href: 'http://something', 'xlink:href': 'resource://something' }))
          ).toBe(false);
          expect(
            allowAutoBootstrap(
              createFakeDoc({ src: 'resource://something', href: 'http://something', 'xlink:href': 'http://something' })
            )
          ).toBe(false);
        });
