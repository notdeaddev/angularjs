describe('ngMock', function () {
  ...
  describe('TzDate', function () {
    ...
    it('should throw error when no third param but toString called', function () {
      expect(function () {
        new angular.mock.TzDate(0, 0).toString();
  ...
  describe('$log', function () {
    angular.forEach([true, false], function (debugEnabled) {
      describe('debug ' + debugEnabled, function () {
        ...
        it(
          'should skip debugging output if disabled (' + debugEnabled + ')',
          inject(function ($log) {
            $log.log('fake log');
            $log.info('fake log');
            $log.warn('fake log');
  ...
  describe('$interval', function () {
    ...
    it('should NOT call $apply if invokeApply is set to false', inject(function ($interval, $rootScope) {
    ...
    it('should return a promise which will be updated with the count on each iteration', inject(function ($interval) {
      var log = [],
    ...
    it('should return a promise which will be resolved after the specified number of iterations', inject(function (
      $interval
    ) {
      var log = [],
        promise = $interval(
      ...
      expect(log).toEqual(['tick', 'promise update: 0', 'tick', 'promise update: 1', 'promise success: 2']);
    }));
    ...
    describe('exception handling', function () {
      ...
      it('should delegate exception to the $exceptionHandler service', inject(function ($interval, $exceptionHandler) {
        $interval(function () {
          throw 'Test Error';
        }, 1000);
      ...
      it('should call $apply even if an exception is thrown in callback', inject(function ($interval, $rootScope) {
        var applySpy = spyOn($rootScope, '$apply').and.callThrough();
      ...
      it('should still update the interval promise when an exception is thrown', inject(function ($interval) {
        var log = [],
          promise = $interval(function () {
    ...
    describe('cancel', function () {
      ...
      it('should not throw a runtime exception when given an undefined promise', inject(function ($interval) {
        var task1 = jasmine.createSpy('task1'),
          promise1;
  ...
  describe('$browser', function () {
    ...
    describe('defer.flush', function () {
      ...
      describe('tasks scheduled during flushing', function () {
        it('should be flushed if they do not exceed the target delay (when no delay specified)', function () {
        ...
        it('should be flushed if they do not exceed the specified delay', function () {
          browser.defer(function () {
            logFn('1')();
            browser.defer(function () {
              logFn('3')();
    ...
    describe('defer.cancel', function () {
      it('should cancel a pending task', function () {
        ...
        expect(log).toBe('');
        expect(function () {
          browser.defer.verifyNoPendingTasks('fooType');
        }).toThrow();
        expect(function () {
    ...
    describe('defer.verifyNoPendingTasks', function () {
      ...
      describe('with specific task type', function () {
        it('should throw if there are pending tasks', function () {
          ...
          expect(function () {
            browser.defer.verifyNoPendingTasks('barType');
          }).not.toThrow();
          expect(function () {
            browser.defer.verifyNoPendingTasks('fooType');
          }).toThrow();
  ...
  describe('$timeout', function () {
    it('should expose flush method that will flush the pending queue of tasks', inject(function ($rootScope, $timeout) {
      var logger = [],
        logFn = function (msg) {
          return function () {
            logger.push(msg);
    ...
    it('should recommend `$verifyNoPendingTasks()` when all pending tasks are not timeouts', inject(function (
      $rootScope,
      $timeout
    ) {
      var extraMessage =
        'None of the pending tasks are timeouts. If you only want to verify ' +
  ...
  describe('$httpBackend', function () {
    ...
    describe('matchLatestDefinitionEnabled()', function () {
      ...
      it('should respond with the first matched definition when false', function () {
      ...
      it('should respond with latest matched definition when true', function () {
        hb.matchLatestDefinitionEnabled(true);
    ...
    describe('respond()', function () {
      ...
      it('should decode query parameters in respond() function', function () {
        hb.expect('GET', '/url?query=l%E2%80%A2ng%20string%20w%2F%20spec%5Eal%20char%24&id=1234&orderBy=-name').respond(
          function (m, u, d, h, p) {
        ...
        hb(
          'GET',
          '/url?query=l%E2%80%A2ng%20string%20w%2F%20spec%5Eal%20char%24&id=1234&orderBy=-name',
          null,
          callback
        );
        hb.flush();
        ...
        expect(callback).toHaveBeenCalledOnceWith(
          200,
          'id=1234;orderBy=-name;query=l•ng string w/ spec^al char$',
          '',
          '',
          undefined
        );
      ...
      it('should include regex captures in respond() params when keys provided', function () {
        hb.expect('GET', /\/(.+)\/article\/(.+)/, undefined, undefined, ['id', 'name']).respond(
          function (m, u, d, h, p) {
    ...
    describe('flush()', function () {
      ...
      it('should throw exception when flushing more requests than pending', function () {
        ...
        expect(function () {
          hb.flush(2);
        }).toThrowError('No more pending request to flush !');
        expect(callback).toHaveBeenCalledOnce();
      });
      ...
      it('should throw exception if not all expectations satisfied', function () {
        ...
        expect(function () {
          hb.flush();
        }).toThrowError('Unsatisfied requests: GET /url2');
      });
    });
    ...
    describe('expectRoute/whenRoute shortcuts', function () {
      angular.forEach(['expectRoute', 'whenRoute'], function (routeShortcut) {
        ...
        they('should provide ' + routeShortcut + ' shortcut with $prop method', methods, function () {
        ...
        they('should match colon delimited parameters in ' + routeShortcut + ' $prop method', methods, function () {
        ...
        they(
          'should ignore query params when matching in ' + routeShortcut + ' $prop method',
          methods,
          function (method) {
            angular.forEach(
              [
                ...
                {
                  route: '/route3/:id',
                  url: '/route3/Charlie?q=str&foo=bar',
                  expectedParams: { id: 'Charlie', q: 'str', foo: 'bar' }
                },
                {
                  route: '/:x/route4',
                  url: '/Delta/route4?q=str&foo=bar',
                  expectedParams: { x: 'Delta', q: 'str', foo: 'bar' }
                },
                {
                  route: '/route5/:id*',
                  url: '/route5/Echo/456?q=str&foo=bar',
                  expectedParams: { id: 'Echo/456', q: 'str', foo: 'bar' }
                },
                {
                  route: '/route6/:id*',
                  url: '/route6/Foxtrot/456/?q=str&foo=bar',
                  expectedParams: { id: 'Foxtrot/456', q: 'str', foo: 'bar' }
                },
                {
                  route: '/route7/:id*',
                  url: '/route7/Golf/456//?q=str&foo=bar',
                  expectedParams: { id: 'Golf/456', q: 'str', foo: 'bar' }
                },
                {
                  route: '/:x*/route8',
                  url: '/Hotel/123/456/route8/?q=str&foo=bar',
                  expectedParams: { x: 'Hotel/123/456', q: 'str', foo: 'bar' }
                },
                {
                  route: '/:x*/route9/:id',
                  url: '/India/456/route9/0?q=str&foo=bar',
                  expectedParams: { x: 'India/456', id: '0', q: 'str', foo: 'bar' }
                },
              ...
              function (testDataEntry) {
                ...
                hb[routeShortcut](method, testDataEntry.route).respond(function (method, url, data, headers, params) {
                  paramsSpy(params);
  ...
  describe('$rootScopeDecorator', function () {
    ...
    describe('$countWatchers', function () {
      it('should return the sum of watchers for the current scope and all of its children', inject(function (
        $rootScope
      ) {
      ...
      it('should provide the current count even after child destruction and watch deregistration', inject(function (
        $rootScope
      ) {
  ...
  describe('$controllerDecorator', function () {
    ...
    it('should support assigning bindings when a value is returned from the constructor', function () {
      var called = false;
      var data = [
        { name: 'derp1', id: 0 },
        { name: 'testname', id: 1 },
...
describe('ngMockE2E', function () {
  ...
  describe('$httpBackend', function () {
    ...
    beforeEach(function () {
      ...
      angular.module('ng').config(function ($provide) {
        ...
        $provide.factory('$httpBackend', [
          '$browser',
          function ($browser) {
            return realHttpBackend.and.callFake(function () {
  ...
  describe('ngAnimateMock', function () {
    ...
    describe('$animate.flush()', function () {
      ...
      it('should trigger the animation to start', inject(function ($animate) {
      ...
      it('should trigger the animation to end once run and called', inject(function ($animate) {
        $animate.leave(element);
        $animate.flush();
      ...
      it('should trigger the animation promise callback to fire once run and closed', inject(function ($animate) {
      ...
      it('should trigger a series of CSS animations to trigger and start once run', inject(function (
        $animate,
        $rootScope
      ) {
      ...
      it('should trigger parent and child animations to run within the same flush', inject(function (
        $animate,
        $rootScope
      ) {
      ...
      it('should trigger animation callbacks when called', inject(function ($animate, $rootScope) {
    ...
    describe('$animate.closeAndFlush()', function () {
      it('should close the currently running $animateCss animations', inject(function ($animateCss, $animate) {
        if (!browserSupportsCssAnimations()) return;
      ...
      it('should close the currently running $$animateJs animations', inject(function ($$animateJs, $animate) {
        var spy = jasmine.createSpy();
      ...
      it('should run the closing javascript animation function upon flush', inject(function ($$animateJs, $animate) {
      ...
      it('should not throw when a regular animation has no javascript animation', inject(function (
        $animate,
        $$animation,
        $rootElement
      ) {
      ...
      it('should throw an error if there are no animations to close and flush', inject(function ($animate) {
        expect(function () {
          $animate.closeAndFlush();
        }).toThrowError('No pending animations ready to be closed or flushed');
      }));
...
describe('`afterEach` clean-up', function () {
  describe('`$rootElement`', function () {
    describe('undecorated', function () {
      ...
      it('should set up spies for the next test to verify that `$rootElement` was cleaned up', function () {
        module(function ($provide) {
    ...
    describe('decorated', function () {
      ...
      it('should set up spies for the next text to verify that `$rootElement` was cleaned up', function () {
        module(function ($provide) {
          $provide.decorator('$rootElement', function ($delegate) {
  ...
  describe('`$rootScope`', function () {
    describe('undecorated', function () {
      ...
      it('should set up spies for the next test to verify that `$rootScope` was cleaned up', inject(function (
        $rootScope
      ) {
...
describe('sharedInjector', function () {
  ...
  it(
    'allows me to mutate a single instance of a module (proving it has been shared)',
    ngMockTest(function () {
      sdescribe('test state is shared', function () {
        angular.module('sharedInjectorTestModuleA', []).factory('testService', function () {
          return { state: 0 };
        });
        ...
        sit(
          'expect mutation to have persisted',
          inject(function (testService) {
  ...
  it(
    ...
    ngMockTest(function () {
      sdescribe('test state is not shared', function () {
        angular.module('sharedInjectorTestModuleC', []).factory('testService', function () {
          return { state: 0 };
        });
        ...
        sit(
          'expect mutation not to have persisted',
          inject(function (testService) {
  ...
  it(
    'allows me to stub with shared injector',
    ngMockTest(function () {
      sdescribe('test state is shared', function () {
        angular.module('sharedInjectorTestModuleD', []).value('testService', 43);

        module.sharedInjector();

        sbeforeAll(
          module('sharedInjectorTestModuleD', function ($provide) {
            $provide.value('testService', 42);
          })
        );

        sit(
          'expected access stubbed value',
          inject(function (testService) {
  ...
  it(
    "doesn't interfere with other test describes",
    ngMockTest(function () {
      angular.module('sharedInjectorTestModuleE', []).factory('testService', function () {
        return { state: 0 };
      });
      ...
      sdescribe('without stubbed injector', function () {
        ...
        sit(
          'expect original, unmutated value',
          inject(function (testService) {
            expect(testService.state).toEqual(0);
          })
        );
