...
describe('$compile', function () {
    ...
    it('should throw an exception if a directive name starts with a non-lowercase letter', function () {
      module(function () {
        expect(function () {
          ...
          '$compile',
          'baddir',
          "Directive/Component name 'BadDirectiveName' is invalid. The first character must be a lowercase letter"
        );
      });
      inject(function ($compile) {});
    });
  ...
  describe('svg namespace transcludes', function () {
    ...
    it('should handle transcluded svg elements', inject(function ($compile) {
      element = jqLite('<div><svg-container><circle cx="4" cy="4" r="2"></circle></svg-container></div>');
    ...
    it('should handle custom svg elements inside svg tag', inject(function () {
      element = jqLite('<div><svg width="300" height="300"><svg-circle></svg-circle></svg></div>');
      $compile(element.contents())($rootScope);
      document.body.appendChild(element[0]);
    ...
    it('should handle transcluded custom svg elements', inject(function () {
      element = jqLite('<div><svg-container><svg-circle></svg-circle></svg-container></div>');
      $compile(element.contents())($rootScope);
      document.body.appendChild(element[0]);
    ...
    if (supportsForeignObject()) {
      ...
      if (!isEdge) {
        it(
          'should handle custom svg containers that transclude to foreignObject' +
            ' that transclude to custom svg containers that transclude to custom elements',
          inject(function () {
            element = jqLite(
              '<div><svg-container>' +
    ...
    it(
      'should support directives with SVG templates and a slow url ' +
  ...
  describe('compile phase', function () {
    ...
    it('should not blow up when elements with no childNodes property are compiled', inject(function (
      $compile,
      $rootScope
    ) {
    ...
    it('should detect anchor elements with the string "SVG" in the `href` attribute as an anchor', inject(function (
      $compile,
      $rootScope
    ) {
      element = jqLite('<div><a href="/ID_SVG_ID"><span ng-if="true">Should render</span></a></div>');
      $compile(element.contents())($rootScope);
      $rootScope.$digest();
      document.body.appendChild(element[0]);
    ...
    describe('multiple directives per element', function () {
      it('should allow multiple directives per element', inject(function ($compile, $rootScope, log) {
        element = $compile('<span greet="angular" log="L" x-high-log="H" data-medium-log="M"></span>')($rootScope);
      ...
      it('should handle exceptions', function () {
        ...
        inject(function ($rootScope, $compile, $exceptionHandler) {
          ...
          expect(sortTag($exceptionHandler.errors[1][1])).toEqual(
            '<div factory-error="" linking-error="" template-error="">'
          );
          expect($exceptionHandler.errors[2][0]).toEqual('LinkingError');
          expect(sortTag($exceptionHandler.errors[2][1])).toEqual(
            '<div class="ng-scope" factory-error="" linking-error="" template-error="">'
          );
    ...
    describe('compiler control', function () {
      describe('priority', function () {
        it('should honor priority', inject(function ($compile, $rootScope, log) {
          element = $compile('<span log="L" x-high-log="H" data-medium-log="M"></span>')($rootScope);
          expect(log).toEqual('L; M; H');
        }));
      });
      ...
      describe('terminal', function () {
        ...
        it('should prevent further directives from running, but finish current priority level', inject(function (
          $rootScope,
          $compile,
          log
        ) {
          ...
          element = $compile('<div high-log medium-stop log class="medium-log"><a set="FAIL">OK</a></div>')($rootScope);
          expect(element.text()).toEqual('OK');
      ...
      describe('restrict', function () {
        it('should allow restriction of availability', function () {
          module(function () {
            forEach({ div: 'E', attr: 'A', clazz: 'C', comment: 'M', all: 'EACM' }, function (restrict, name) {
      ...
      describe('template', function () {
        ...
        it('should merge attributes including style attr', inject(function ($compile, $rootScope) {
          element = $compile('<div><div replace class="medium-log" style="height: 20px" ></div><div>')($rootScope);
        ...
        it('should not merge attributes if they are the same', inject(function ($compile, $rootScope) {
          element = $compile('<div><div nomerge class="medium-log" id="myid"></div><div>')($rootScope);
          var div = element.find('div');
          expect(div.hasClass('medium-log')).toBe(true);
          expect(div.hasClass('log')).toBe(true);
        ...
        it('should not add white-space when merging an attribute that is "" in the replaced element', inject(function (
          $compile,
          $rootScope
        ) {
          element = $compile('<div><div replace class=""></div><div>')($rootScope);
          var div = element.find('div');
          expect(div.hasClass('log')).toBe(true);
        ...
        it('should not set merged attributes twice in $attrs', function () {
          ...
          inject(function ($compile, $rootScope) {
            element = $compile('<div><div log-attrs replace class="myLog"></div><div>')($rootScope);
            var div = element.find('div');
        ...
        it('should play nice with repeater when replacing', inject(function ($compile, $rootScope) {
          element = $compile('<div><div ng-repeat="i in [1,2]" replace></div></div>')($rootScope);
          $rootScope.$digest();
        ...
        it('should play nice with repeater when appending', inject(function ($compile, $rootScope) {
          element = $compile('<div><div ng-repeat="i in [1,2]" append></div></div>')($rootScope);
          $rootScope.$digest();
          expect(element.text()).toEqual('Append!Append!');
        }));

        it('should handle interpolated css class from replacing directive', inject(function ($compile, $rootScope) {
        ...
        if (!msie) {
          ...
          it('should handle interpolated css style from replacing directive', inject(function ($compile, $rootScope) {
        ...
        it('should merge interpolated css class with ngRepeat', inject(function ($compile, $rootScope) {
          element = $compile('<div><div ng-repeat="i in [1]" class="one {{cls}} three" replace></div></div>')(
            $rootScope
          );
        ...
        it('should interpolate the values once per digest', inject(function ($compile, $rootScope, log) {
        ...
        describe('replace and not exactly one root element', function () {
          ...
          they(
            'should not throw if the root element is accompanied by: $prop',
            {
      ...
      describe('template as function', function () {
        ...
        it('should evaluate `template` when defined as fn and use returned string as template', inject(function (
          $compile,
          $rootScope
        ) {
      ...
      describe('templateUrl', function () {
        ...
        it('should not load cross domain templates by default', inject(function ($compile, $rootScope) {
          expect(function () {
            $compile('<div class="crossDomainTemplate"></div>')($rootScope);
          }).toThrowMinErr(
            '$sce',
            'insecurl',
            'Blocked loading resource from url not allowed by $sceDelegate policy.  URL: http://example.com/should-not-load.html'
          );
        }));

        it('should trust what is already in the template cache', inject(function (
          $compile,
          $httpBackend,
          $rootScope,
          $templateCache
          ...
          $httpBackend
            .expect('GET', 'http://example.com/should-not-load.html')
            .respond('<span>example.com/remote-version</span>');
        ...
        it('should load cross domain templates when trusted', inject(function (
          $compile,
          $httpBackend,
          $rootScope,
          $sce
          ...
          $httpBackend
            .expect('GET', 'http://example.com/trusted-template.html')
            .respond('<span>example.com/trusted_template_contents</span>');
          element = $compile('<div class="trustedTemplate"></div>')($rootScope);
          expect(sortedHtml(element)).toEqual('<div class="trustedTemplate"></div>');
          $httpBackend.flush();
          expect(sortedHtml(element)).toEqual(
            '<div class="trustedTemplate"><span>example.com/trusted_template_contents</span></div>'
          );
        }));

        it('should append template via $http and cache it in $templateCache', inject(function (
          $compile,
          $httpBackend,
          $templateCache,
          $rootScope,
        ...
        it('should inline template via $http and cache it in $templateCache', inject(function (
          $compile,
          $httpBackend,
          $templateCache,
          $rootScope
        ...
        it('should compile, link and flush the template append', inject(function (
          $compile,
          $templateCache,
          $rootScope,
          $browser
        ...
        it('should compile, link and flush the template inline', inject(function (
          $compile,
          $templateCache,
          $rootScope
        ) {
        ...
        it('should compile, flush and link the template append', inject(function (
          $compile,
          $templateCache,
          $rootScope
        ) {
        ...
        it('should compile, flush and link the template inline', inject(function (
          $compile,
          $templateCache,
          $rootScope
        ) {
        ...
        it('should compile template when replacing element in another template', inject(function (
          $compile,
          $templateCache,
          $rootScope
        ) {
        ...
        it('should compile template when replacing root element', inject(function (
          $compile,
          $templateCache,
          $rootScope
        ) {
        ...
        it('should resolve widgets after cloning in append mode', function () {
          ...
          inject(function ($compile, $templateCache, $rootScope, $httpBackend, $browser, $exceptionHandler) {
        ...
        it('should be implicitly terminal and not compile placeholder content in append', inject(function (
          $compile,
          $templateCache,
          $rootScope,
          log
        ...
        it('should be implicitly terminal and not compile placeholder content in inline', inject(function (
          $compile,
          $templateCache,
          $rootScope,
          log
        ...
        it('should throw an error and clear element content if the template fails to load', inject(function (
          $compile,
          $httpBackend,
          $rootScope
        ) {
        ...
        it('should prevent multiple templates per element', function () {
          ...
          inject(function ($compile, $httpBackend) {
            ...
            expect(function () {
              ...
              '$compile',
              'multidir',
              'Multiple directives [async, sync] asking for template on: <div class="sync async">'
            );
          });
        });
        ...
        describe('delay compile / linking functions until after template is resolved', function () {
          ...
          it('should flush after link append', inject(function ($compile, $rootScope, $httpBackend, log) {
          ...
          it('should flush after link inline', inject(function ($compile, $rootScope, $httpBackend, log) {
          ...
          it('should flush before link append', inject(function ($compile, $rootScope, $httpBackend, log) {
            $httpBackend.expect('GET', 'second.html').respond('<div third>{{1+2}}</div>');
            template = $compile('<div><span first second last></span></div>');
            expect(log).toEqual('first-C');
            log('FLUSH');
          ...
          it('should flush before link inline', inject(function ($compile, $rootScope, $httpBackend, log) {
            $httpBackend.expect('GET', 'second.html').respond('<div i-third>{{1+2}}</div>');
            template = $compile('<div><span i-first i-second i-last></span></div>');
            expect(log).toEqual('iFirst-C');
            log('FLUSH');
        ...
        it('should work when directive is on the root element', inject(function ($compile, $httpBackend, $rootScope) {
            ...
            function (directiveTemplate) {
              inject(function ($compile, $templateCache, $rootScope) {
                ...
                expect(function () {
                  ...
                  '$compile',
                  'tplrt',
                  "Template for directive 'template' must have exactly one root element. template.html"
                );
              });
            }
          );
        ...
        it('should support templates with root <optgroup> tags', inject(function (
          $compile,
          $rootScope,
          $templateCache
        ) {
      ...
      describe('templateUrl as function', function () {
        ...
        it('should evaluate `templateUrl` when defined as fn and use returned value as url', inject(function (
          $compile,
          $rootScope,
          $templateCache
        ) {
      ...
      describe('scope', function () {
        ...
        it('should allow creation of new isolated scopes for directives', inject(function ($rootScope, $compile, log) {
        ...
        it('should allow creation of new scopes for directives with templates', inject(function (
          $rootScope,
          $compile,
          log,
          $httpBackend
        ...
        it('should allow creation of new scopes for replace directives with templates', inject(function (
          $rootScope,
          $compile,
          log,
          $httpBackend
        ...
        it('should allow creation of new scopes for replace directives with templates in a repeater', inject(function (
          $rootScope,
          $compile,
          log,
          $httpBackend
        ...
        it('should allow creation of new isolated scopes for directives with templates', inject(function (
          $rootScope,
          $compile,
          log,
          $httpBackend
        ...
        it('should correctly create the scope hierarchy', inject(function ($rootScope, $compile, log) {
          element = $compile(
            '<div>' + //1
              '<b class=scope>' + //2
        ...
        it('should allow more than one new scope directives per element, but directives should sharethe scope', inject(function (
          $rootScope,
          $compile,
          log
        ) {
        ...
        it('should not allow more than one isolate scope creation per element', inject(function ($rootScope, $compile) {
          expect(function () {
            $compile('<div class="iscope-a; scope-b"></div>');
          }).toThrowMinErr(
            '$compile',
            'multidir',
            'Multiple directives [iscopeA, scopeB] asking for new/isolated scope on: ' +
              '<div class="iscope-a; scope-b">'
          );
        }));

        it('should not allow more than one isolate/new scope creation per element regardless of `templateUrl`', inject(function (
          $httpBackend
        ) {
        ...
        it('should not allow more than one isolate scope creation per element regardless of directive priority', function () {
          ...
          inject(function ($compile) {
            expect(function () {
              ...
              '$compile',
              'multidir',
              'Multiple directives [highPriorityScope, iscopeA] asking for new/isolated scope on: ' +
        ...
        it('should create isolate scope even at the root of the template', inject(function ($rootScope, $compile, log) {
        ...
        describe('scope()/isolate() scope getters', function () {
          describe('with no directives', function () {
            it('should return the scope of the parent node', inject(function ($rootScope, $compile) {
              element = $compile('<div></div>')($rootScope);
          ...
          describe('with new scope directives', function () {
            it('should return the new scope at the directive element', inject(function ($rootScope, $compile) {
              element = $compile('<div scope></div>')($rootScope);
              expect(element.scope().$parent).toBe($rootScope);
            }));

            it('should return the new scope for children in the original template', inject(function (
              $rootScope,
              $compile
            ) {
            ...
            it('should return the new scope for children in the directive template', inject(function (
              $rootScope,
              $compile,
              $httpBackend
            ) {
            ...
            it('should return the new scope for children in the directive sync template', inject(function (
              $rootScope,
              $compile
            ) {
          ...
          describe('with isolate scope directives', function () {
            it('should return the root scope for directives at the root element', inject(function (
              $rootScope,
              $compile
            ) {
              element = $compile('<div iscope></div>')($rootScope);
            ...
            it('should return the non-isolate scope at the directive element', inject(function ($rootScope, $compile) {
            ...
            it('should return the isolate scope for children in the original template', inject(function (
              $rootScope,
              $compile
            ) {
            ...
            it('should return the isolate scope for children in directive template', inject(function (
              $rootScope,
              $compile,
              $httpBackend
            ) {
            ...
            it('should return the isolate scope for children in directive sync template', inject(function (
              $rootScope,
              $compile
            ) {
            ...
            it('should handle "=" bindings with same method names in Object.prototype correctly when not present', inject(function (
              $rootScope,
              $compile
            ) {
              var func = function () {
                element = $compile('<div prototype-method-name-as-scope-var-a></div>')($rootScope);
              };
            ...
            it('should handle "=" bindings with same method names in Object.prototype correctly when present', inject(function (
              $rootScope,
              $compile
            ) {
            ...
            it(
              ...
              function () {
                ...
                inject(function ($rootScope, $compile) {
                  ...
                  expect(func).toThrowMinErr(
                    '$compile',
                    'missingattr',
                    "Attribute 'valueOf' of 'prototypeMethodNameAsScopeVarA' is non-optional and must be set!"
                  );
                });
              }
            );
            ...
            it(
              ...
              function () {
                ...
                inject(function ($rootScope, $compile) {
                  var func = function () {
                    element = $compile('<div prototype-method-name-as-scope-var-a value-of="valueOf"></div>')(
                      $rootScope
                    );
                  };
                  expect(func).not.toThrow();
              ...
              function () {
                ...
                inject(function ($rootScope, $compile) {
                  ...
                  expect(func).toThrowMinErr(
                    '$compile',
                    'missingattr',
                    "Attribute 'valueOf' of 'prototypeMethodNameAsScopeVarB' is non-optional and must be set!"
                  );
                });
              }
            );
            ...
            it(
              ...
              function () {
                ...
                inject(function ($rootScope, $compile) {
                  var func = function () {
                    element = $compile('<div prototype-method-name-as-scope-var-b value-of="valueOf"></div>')(
                      $rootScope
                    );
                  };
                  expect(func).not.toThrow();
              ...
              function () {
                ...
                inject(function ($rootScope, $compile) {
                  ...
                  expect(func).toThrowMinErr(
                    '$compile',
                    'missingattr',
                    "Attribute 'valueOf' of 'prototypeMethodNameAsScopeVarC' is non-optional and must be set!"
                  );
                });
              }
            );
            ...
            it(
              ...
              function () {
                ...
                inject(function ($rootScope, $compile) {
                  var func = function () {
                    element = $compile('<div prototype-method-name-as-scope-var-c value-of="valueOf"></div>')(
                      $rootScope
                    );
                  };
                  expect(func).not.toThrow();
            ...
            it(
              ...
              function () {
                ...
                inject(function ($rootScope, $compile) {
                  var func = function () {
                    element = $compile('<div prototype-method-name-as-scope-var-d></div>')($rootScope);
                  };
                  expect(func).toThrowMinErr(
                    '$compile',
                    'missingattr',
                    "Attribute 'valueOf' of 'prototypeMethodNameAsScopeVarD' is non-optional and must be set!"
                  );
                });
              }
            );
            ...
            it(
              ...
              function () {
                ...
                inject(function ($rootScope, $compile) {
                  var func = function () {
                    element = $compile('<div prototype-method-name-as-scope-var-d value-of="valueOf"></div>')(
                      $rootScope
                    );
                  };
                  expect(func).not.toThrow();
            ...
            it('should not throw exception when using "watch" as binding in Firefox', inject(function (
              $rootScope,
              $compile
            ) {
              $rootScope.watch = 'watch';
              var func = function () {
                element = $compile('<div watch-as-scope-var watch="watch"></div>')($rootScope);
              };
          ...
          describe('with isolate scope directives and directives that manually create a new scope', function () {
            ...
            it('should return the isolate scope for child elements', inject(function (
              $rootScope,
              $compile,
              $httpBackend
            ) {
            ...
            it('should return the isolate scope for child elements in directive sync template', inject(function (
              $rootScope,
              $compile
            ) {
              var directiveElement, child;
        ...
        describe('multidir isolated scope error messages', function () {
          ...
          beforeEach(
            module('fakeIsoledScopeModule', function () {
          ...
          it('should add module name to multidir isolated scope message if directive defined through module', inject(function (
            $rootScope,
            $compile
          ) {
            expect(function () {
          ...
          it("shouldn't add module name to multidir isolated scope message if directive is defined directly with $compileProvider", inject(function (
            $rootScope,
            $compile
          ) {
            expect(function () {
  ...
  describe('interpolation', function () {
    ...
    it('should compile and link both attribute and text bindings', inject(function ($rootScope, $compile) {
    ...
    it('should one-time bind if the expression starts with a space and two colons', inject(function (
      $rootScope,
      $compile
    ) {
      $rootScope.name = 'angular';
    ...
    describe('SCE values', function () {
      it('should resolve compile and link both attribute and text bindings', inject(function (
        $rootScope,
        $compile,
        $sce
      ) {
    ...
    it('should allow directive to replace interpolated attributes before attr interpolation compilation', inject(function (
      $compile,
      $rootScope
    ) {
    ...
    it('should call observer of non-interpolated attr through $evalAsync', inject(function ($rootScope, $compile) {
    ...
    it('should support non-interpolated `src` and `data-src` on the same element', inject(function (
      $rootScope,
      $compile
    ) {
    ...
    it('should translate {{}} in terminal nodes', inject(function ($rootScope, $compile) {
      ...
      $rootScope.$digest();
      expect(sortedHtml(element).replace(' selected="selected"', '')).toEqual(
        '<select ng:model="x"><option value="">Greet !</option></select>'
      );
      $rootScope.name = 'Misko';
      $rootScope.$digest();
      expect(sortedHtml(element).replace(' selected="selected"', '')).toEqual(
        '<select ng:model="x"><option value="">Greet Misko!</option></select>'
      );
    }));
    ...
    it('should support custom start/end interpolation symbols in template and directive template', function () {
    ...
    it("should support custom start interpolation symbol, even when `endSymbol` doesn't change", function () {
    ...
    it("should support custom end interpolation symbol, even when `startSymbol` doesn't change", function () {
      module(function ($compileProvider, $interpolateProvider) {
    ...
    it('should support custom start/end interpolation symbols in async directive template', function () {
      module(function ($interpolateProvider, $compileProvider) {
        $interpolateProvider.startSymbol('##').endSymbol(']]');
        $compileProvider.directive('myDirective', function () {
          return {
  ...
  describe('link phase', function () {
    ...
    it('should compile from top to bottom but link from bottom up', inject(function ($compile, $rootScope, log) {
  ...
  describe('attrs', function () {
    ...
    it('should properly $observe inside ng-repeat', function () {
      ...
      inject(function ($compile, $rootScope) {
        element = $compile('<div><div ng-repeat="i in items"><span some="id_{{i.id}}" observer></span></div></div>')(
          $rootScope
        );
  ...
  describe('controller lifecycle hooks', function () {
    ...
    describe('$onChanges', function () {
      ...
      it('should trigger `$onChanges` for literal expressions when expression input value changes (simple value)', function () {
        ...
        inject(function ($compile, $rootScope) {
          ...
          expect(log.pop()).toEqual({
            prop1: jasmine.objectContaining({ previousValue: [undefined], currentValue: [1] })
          });
      ...
      it('should not call `$onChanges` twice even when the initial value is `NaN`', function () {
        ...
        inject(function ($compile, $rootScope) {
          var template = '<test prop="a" attr="{{a}}"></test><test prop="b" attr="{{b}}"></test>';
          $rootScope.a = 'foo';
          $rootScope.b = NaN;
  ...
  describe('isolated locals', function () {
    ...
    it('should be able to bind attribute names which are present in Object.prototype', function () {
      ...
      inject(function ($rootScope) {
        expect(function () {
          compile('<div in-proto-attr constructor="hello, world" watch="[]" to-string="value = !value"></div>');
        }).not.toThrow();
        var isolateScope = element.isolateScope();
    ...
    it('should not initialize scope value if optional expression binding with Object.prototype name is not passed', inject(function (
      $compile
    ) {
      compile('<div my-component></div>');
      var isolateScope = element.isolateScope();
    ...
    it('should initialize scope value if optional expression binding with Object.prototype name is passed', inject(function (
      $compile
    ) {
    ...
    describe('bind-once', function () {
      ...
      it('should continue with a digets cycle when there is a two-way binding from the child to the parent', function () {
        ...
        inject(function ($rootScope) {
          compile('<div><p>{{greeting}}</p><div><hello greeting="greeting"></hello></div></div>');
          $rootScope.$digest();
    ...
    describe('one-way binding', function () {
      ...
      describe('initialization', function () {
        ...
        it('should not update isolate again after $onInit if outer is a literal', function () {
          ...
          inject(function () {
            ...
            expect(log).toEqual([
              ...
              ['$onChanges', jasmine.objectContaining({ currentValue: ['outer'] })],
              '$onInit',
              ['$onChanges', jasmine.objectContaining({ previousValue: ['outer'], currentValue: ['re-outer'] })]
            ]);
          });
        });
        ...
        it('should update isolate again after $onInit if outer has changed (before initial watchAction call)', function () {
          angular.module('owComponentTest').directive('changeInput', function () {
    ...
    it('should expose isolate scope variables on controller with controllerAs when bindToController is true (template)', function () {
      ...
      inject(function ($compile, $rootScope) {
        ...
        element = $compile('<div foo-dir dir-data="remoteData" dir-str="Hello, {{whom}}!" dir-fn="fn()"></div>')(
          $rootScope
        );
    ...
    it('should throw noctrl when missing controller', function () {
      ...
      inject(function ($compile, $rootScope) {
        ...
        }).toThrowMinErr('$compile', 'noctrl', "Cannot bind to controller without directive 'noCtrl's controller.");
      });
    });
          ...
          forEach(templateOptions, function (templateOption) {
            ...
            it('(' + description.join(', ') + ')', function () {
              ...
              inject(function ($compile, $rootScope, $templateCache) {
                ...
                element = $compile(
                  '<div foo-dir dir-data="remoteData" dir-str="Hello, {{whom}}!" dir-fn="fn()"></div>'
                )($rootScope);
                $rootScope.$digest();
                expect(controllerCalled).toBe(true);
    ...
    it('should evaluate against the correct scope, when using `bindToController` (new scope)', function () {
      module(function ($compileProvider, $controllerProvider) {
    ...
    it('should evaluate against the correct scope, when using `bindToController` (new iso scope)', function () {
      module(function ($compileProvider, $controllerProvider) {
        $controllerProvider.register({
          ParentCtrl: function () {
            this.value1 = 'parent1';
  ...
  describe('require', function () {
    ...
    it('should require controller of an isolate directive from a non-isolate directive on the same element', function () {
    ...
    it('should not get confused about where to use isolate scope when a replaced directive is used multiple times', function () {
      module(function () {
        directive('isolate', function () {
          return {
            replace: true,
      ...
      inject(function ($compile, $rootScope, log) {
        element = $compile('<div><div isolate scope-tester="outside"></div><span scope-tester="sibling"></span></div>')(
          $rootScope
        );
    ...
    it('should require controller of a non-isolate directive from an isolate directive on the same element', function () {
    ...
    it('should support omitting the name of the required controller if it is the same as the key', function () {
      module(function () {
        directive(
          'myC1',
          valueFn({
    ...
    it('should instantiate the controller just once when template/templateUrl', function () {
      ...
      inject(function ($templateCache, $compile, $rootScope) {
        ...
        element = $compile('<div><span xmy-directive-sync></span><span my-directive-async></span></div>')($rootScope);
        expect(syncCtrlSpy).not.toHaveBeenCalled();
        expect(asyncCtrlSpy).not.toHaveBeenCalled();
    ...
    it('should throw ctreq with correct directive name, regardless of order', function () {
      ...
      inject(function ($compile, $rootScope) {
        ...
        }).toThrowMinErr('$compile', 'ctreq', "Controller 'ngModel', required by directive 'aDir', can't be found!");
      });
    });
  });
  ...
  describe('transclude', function () {
    describe('content transclusion', function () {
      ...
      it('should correctly handle multi-element directives', function () {
        ...
        inject(function ($compile, $rootScope) {
          var tmplWithFoo = '<foo><div ng-if-start="true">Hello, </div><div ng-if-end>world!</div></foo>';
          var tmplWithBar =
            '<bar>' +
      ...
      it('should not process top-level transcluded text nodes merged into their sibling', function () {
        module(function () {
          directive(
            'transclude',
            valueFn({
      ...
      describe('passing a parent bound transclude function to the link function returned from `$compile`', function () {
        beforeEach(
          module(function () {
      ...
      describe('removing a transcluded element', function () {
        ...
        it('should not leak the transclude scope when the transcluded content is an element transclusion directive', inject(function (
          $compile,
          $rootScope
        ) {
          element = $compile('<div toggle><div ng:repeat="msg in [\'msg-1\']">{{ msg }}</div></div>')($rootScope);
        ...
        it('should not leak the transclude scope when the transcluded content is an multi-element transclusion directive', inject(function (
          $compile,
          $rootScope
        ) {
          element = $compile(
        ...
        it('should not leak the transclude scope if the transcluded contains only comments', inject(function (
          $compile,
          $rootScope
        ) {
          element = $compile('<div toggle><!-- some comment --></div>')($rootScope);
        ...
        it('should not leak the transclude scope if the transcluded contains only text nodes', inject(function (
          $compile,
          $rootScope
        ) {
        ...
        it('should mark as destroyed all sub scopes of the scope being destroyed', inject(function (
          $compile,
          $rootScope
        ) {
          element = $compile('<div toggle><div ng:repeat="msg in [\'msg-1\']">{{ msg }}</div></div>')($rootScope);
      ...
      describe('nested transcludes', function () {
        ...
        it('should allow nested transclude directives with sync template containing sync template', inject(function (
          $compile,
          $rootScope
        ) {
        ...
        it('should allow nested transclude directives with sync template containing async template', inject(function (
          $compile,
          $rootScope
        ) {
        ...
        it('should allow nested transclude directives with async template containing sync template', inject(function (
          $compile,
          $rootScope
        ) {
        ...
        it('should allow nested transclude directives with async template containing asynch template', inject(function (
          $compile,
          $rootScope
        ) {
      ...
      describe('nested isolated scope transcludes', function () {
        ...
        it('should pass the outer scope to the transclude on the isolated template sync-sync', inject(function (
          $compile,
          $rootScope
        ) {
        ...
        it('should pass the outer scope to the transclude on the isolated template async-sync', inject(function (
          $compile,
          $rootScope
        ) {
          $rootScope.val = 'transcluded content';
        ...
        it('should pass the outer scope to the transclude on the isolated template async-async', inject(function (
          $compile,
          $rootScope
        ) {
          $rootScope.val = 'transcluded content';
    ...
    describe('element transclusion', function () {
      ...
      it('should only allow one element transclusion per element', function () {
        ...
        inject(function ($compile) {
          expect(function () {
            ...
            '$compile',
            'multidir',
            'Multiple directives [first, second] asking for transclusion on: <!-- first: -->'
          );
        });
      });
      ...
      it('should terminate compilation only for element transclusion', function () {
        ...
        inject(function (log, $compile, $rootScope) {
          $compile('<div><div element-trans log="elem"></div><div regular-trans log="regular"></div></div>')(
            $rootScope
          );
      ...
      it('should instantiate high priority controllers only once, but low priority ones each time we transclude', function () {
        module(function () {
          directive('elementTrans', function (log) {
            return {
              transclude: 'element',
    ...
    it('should safely create transclude comment node and not break with "-->"', inject(function ($rootScope) {
  ...
  describe('multi-slot transclude', function () {
    ...
    it('should error if a required transclude slot is not filled', function () {
      ...
      inject(function ($rootScope, $compile) {
        expect(function () {
          element = $compile('<minion-component><minion>stuart</minion><span>dorothy</span></minion-component>')(
            $rootScope
          );
      ...
      module(function () {
        directive('foo', function () {
          return {
            ...
            template: '<div class="a" ng-transclude="fooBarSlot"></div><div class="b" ng-transclude="mooKarSlot"></div>'
          };
        });
      });
      inject(function ($rootScope, $compile) {
  ...
  describe('img[srcset] sanitization', function () {
    it('should not error if srcset is undefined', function () {
      ...
      module(function () {
        directive(
          'setter',
          valueFn(function (scope, elem, attrs) {
    ...
    it('should NOT work with trusted values', inject(function ($rootScope, $compile, $sce) {
      ...
      $rootScope.testUrl = $sce.trustAsUrl('javascript:something');
      $rootScope.$digest();
      expect(element.attr('srcset')).toEqual('unsafe:javascript:something ,unsafe:javascript:something');
    }));
  ...
  describe('interpolation on HTML DOM event handler attributes onclick, onXYZ, formaction', function () {
    it('should disallow interpolation on onclick', inject(function ($compile, $rootScope) {
      ...
      }).toThrowMinErr('$compile', 'nodomevents', 'Interpolations for HTML DOM event attributes are disallowed');
      expect(function () {
        $compile('<button ONCLICK="{{onClickJs}}"></button>');
      }).toThrowMinErr('$compile', 'nodomevents', 'Interpolations for HTML DOM event attributes are disallowed');
      expect(function () {
    ...
    it('should pass through arbitrary values on onXYZ event attributes that contain a hyphen', inject(function (
      $compile,
      $rootScope
    ) {
  ...
  describe('form[action]', function () {
    ...
    it('should pass through $sce.trustAsResourceUrl() values in action attribute', inject(function (
      $compile,
      $rootScope,
      $sce
    ) {
  ...
  describe('ngAttr* attribute binding', function () {
    ...
    describe('in directive', function () {
      ...
      it('should provide post-digest value in synchronous directive link functions when after overridden attribute', function () {
        $rootScope.test = 'TEST';
      ...
      it('should provide post-digest value in synchronous directive link functions when before overridden attribute', function () {
        $rootScope.test = 'TEST';
      ...
      it('should provide post-digest value in asynchronous directive link functions when after overridden attribute', function () {
        $rootScope.test = 'TEST';
      ...
      it('should provide post-digest value in asynchronous directive link functions when before overridden attribute', function () {
        $rootScope.test = 'TEST';
    ...
    it('should work with different prefixes', inject(function () {
      $rootScope.name = 'Misko';
      element = $compile('<span ng:attr:test="{{name}}" ng-Attr-test2="{{name}}" ng_Attr_test3="{{name}}"></span>')(
        $rootScope
      );
      expect(element.attr('test')).toBeUndefined();
  ...
  describe('multi-element directive', function () {
    it('should group on link function', inject(function ($compile, $rootScope) {
      ...
      element = $compile('<div><span ng-show-start="show"></span><span ng-show-end></span></div>')($rootScope);
      $rootScope.$digest();
      var spans = element.find('span');
    ...
    it('should group on compile function', inject(function ($compile, $rootScope) {
      $rootScope.show = false;
      element = $compile(
        '<div><span ng-repeat-start="i in [1,2]">{{i}}A</span><span ng-repeat-end>{{i}}B;</span></div>'
      )($rootScope);
      $rootScope.$digest();
    ...
    it('should set up and destroy the transclusion scopes correctly', inject(function ($compile, $rootScope) {
      element = $compile(
        '<div>' +
    ...
    it('should throw error if unterminated (containing termination as a child)', function () {
      ...
      inject(function ($compile) {
        expect(function () {
          element = $compile('<div><span foo-start><span foo-end></span></span></div>');
        }).toThrowMinErr(
          '$compile',
          'uterdir',
          "Unterminated attribute, found 'foo-start' but no matching 'foo-end' found."
  ...
  describe('$animate animation hooks', function () {
    ...
    it('should automatically fire the addClass and removeClass animation hooks', inject(function (
      $compile,
      $animate,
      $rootScope
    ) {
      var data,
        element = jqLite('<div class="{{val1}} {{val2}} fire"></div>');
      $compile(element)($rootScope);
