...
describe('$anchorScroll', function () {
  ...
  function simulateDocumentLoaded() {
    return spyOnJQLiteDocumentLoaded(function (callback) {
      callback();
    });
  }
  ...
  describe('when explicitly called', function () {
    ...
    describe('and implicitly using `$location.hash()`', function () {
      it('should scroll to top of the window if empty hash', inject(changeHashAndScroll(''), expectScrollingToTop));
    ...
    describe('and specifying a hash', function () {
      ...
      it('should scroll to top of the window if empty hash', inject(callAnchorScroll(''), expectScrollingToTop));
  ...
  describe('watcher', function () {
    ...
    describe('when document has completed loading', function () {
      ...
      it('should scroll to element when hash change in hashbang mode', function () {
        ...
        inject(addElements('id=some'), changeHashTo('some'), expectScrollingTo('id=some'));
      });
      ...
      it('should not scroll to the top if $anchorScroll is initializing and location hash is empty', inject(
        expectNoScrolling()
      ));

      it('should not scroll when element does not exist', function () {
        module(initLocation({ html5Mode: false, historyApi: false }));
        inject(addElements('id=some'), changeHashTo('other'), expectNoScrolling());
      });
      ...
      it('should not scroll when auto-scrolling is disabled', function () {
        module(disableAutoScrolling(), initLocation({ html5Mode: false, historyApi: false }));
        inject(addElements('id=fake'), changeHashTo('fake'), expectNoScrolling());
      });
  ...
  describe('yOffset', function () {
        ...
        it('should scroll with vertical offset', function () {
          ...
          inject(
            ...
            expectScrollingWithOffset(
              ...
              [1 * increment, 2 * increment, 3 * increment, 4 * increment]
            )
          );
        });
      });
    ...
    describe('and body with border/margin/padding', function () {
      ...
      it('should use the correct vertical offset when changing `yOffset` at runtime', inject(
        addElements('id=some'),
        mockBoundingClientRect({ 0: [yOffsetNumber - necessaryYOffset, yOffsetNumber - necessaryYOffset] }),
        changeHashTo('some'),
        setYOffset(yOffsetNumber - 10),
        callAnchorScroll(),
