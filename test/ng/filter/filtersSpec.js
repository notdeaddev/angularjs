...
describe('filters', function () {
  ...
  describe('formatNumber', function () {
    ...
    it('should work with numbers that are close to the limit for exponent notation', function () {
      ...
      expect(formatNumber(localLimitMax, pattern, ',', '.', 2)).toBe('999,999,999,999,999,900,000.00');
      expect(formatNumber(localLimitMin, pattern, ',', '.', 2)).toBe('10,000,000,000,000,000,000.00');
      expect(formatNumber(exampleNumber, pattern, ',', '.', 2)).toBe('444,444,444,400,000,000,000.00');
    });
  ...
  describe('currency', function () {
    ...
    it('should trim whitespace around the currency symbol if it is empty', inject(function ($locale) {
  ...
  describe('date', function () {
    ...
    it('should accept various format strings', function () {
      expect(date(secondWeek, 'yyyy-Ww')).toEqual('2013-W2');

      expect(date(secondWeek, 'yyyy-Www')).toEqual('2013-W02');

      expect(date(morning, 'yy-MM-dd HH:mm:ss')).toEqual('10-09-03 07:05:08');

      expect(date(morning, 'yy-MM-dd HH:mm:ss.sss')).toEqual('10-09-03 07:05:08.001');

      expect(date(midnight, 'yyyy-M-d h=H:m:saZ')).toEqual('2010-9-3 12=0:5:8AM-0500');

      expect(date(midnight, 'yyyy-MM-dd hh=HH:mm:ssaZ')).toEqual('2010-09-03 12=00:05:08AM-0500');

      expect(date(midnight, 'yyyy-MM-dd hh=HH:mm:ss.sssaZ')).toEqual('2010-09-03 12=00:05:08.123AM-0500');

      expect(date(noon, 'yyyy-MM-dd hh=HH:mm:ssaZ')).toEqual('2010-09-03 12=12:05:08PM-0500');

      expect(date(noon, 'yyyy-MM-dd hh=HH:mm:ss.sssaZ')).toEqual('2010-09-03 12=12:05:08.012PM-0500');

      expect(date(noon, 'EEE, MMM d, yyyy')).toEqual('Fri, Sep 3, 2010');

      expect(date(noon, 'EEEE, MMMM dd, yyyy')).toEqual('Friday, September 03, 2010');

      expect(date(earlyDate, 'MMMM dd, y')).toEqual('September 03, 1');

      expect(date(earlyDate, 'MMMM dd, yyyy')).toEqual('September 03, 0001');

      expect(date(year0Date, 'dd MMMM y G')).toEqual('25 December 1 BC');

      expect(date(bcDate, 'dd MMMM y G')).toEqual('16 January 27 BC');

      expect(date(noon, 'MMMM dd, y G')).toEqual('September 03, 2010 AD');

      expect(date(noon, 'MMMM dd, y GG')).toEqual('September 03, 2010 AD');

      expect(date(noon, 'MMMM dd, y GGG')).toEqual('September 03, 2010 AD');

      expect(date(noon, 'MMMM dd, y GGGG')).toEqual('September 03, 2010 Anno Domini');
    });
    ...
    it('should format timezones correctly (as per ISO_8601)', function () {
      ...
      expect(date(utc, 'yyyy-MM-ddTHH:mm:ssZ')).toEqual('2010-09-03T12:05:08+0000');

      expect(date(eastOfUTC, 'yyyy-MM-ddTHH:mm:ssZ')).toEqual('2010-09-03T17:05:08+0500');

      expect(date(westOfUTC, 'yyyy-MM-ddTHH:mm:ssZ')).toEqual('2010-09-03T07:05:08-0500');

      expect(date(eastOfUTCPartial, 'yyyy-MM-ddTHH:mm:ssZ')).toEqual('2010-09-03T17:35:08+0530');

      expect(date(westOfUTCPartial, 'yyyy-MM-ddTHH:mm:ssZ')).toEqual('2010-09-03T06:35:08-0530');
    });
    ...
    it('should treat single quoted strings as string literals', function () {
      expect(date(midnight, "yyyy'de' 'a'x'dd' 'adZ' h=H:m:saZ")).toEqual('2010de axdd adZ 12=0:5:8AM-0500');
    });

    it('should treat a sequence of two single quotes as a literal single quote', function () {
      expect(date(midnight, "yyyy'de' 'a''dd' 'adZ' h=H:m:saZ")).toEqual("2010de a'dd adZ 12=0:5:8AM-0500");
      expect(date(midnight, "EEE, MMM d, ''yy")).toEqual("Fri, Sep 3, '10");
    });
    ...
    it('should use UTC if the timezone is set to "UTC"', function () {
      ...
      expect(date(new Date(Date.UTC(2003, 8, 10, 3, 2, 4)), 'yyyy-MM-dd HH-mm-ss', 'UTC')).toEqual(
        '2003-09-10 03-02-04'
      );
      expect(date(new Date(Date.UTC(2003, 8, 10, 3, 2, 4)), 'yyyy-MM-dd HH-mm-ssZ', 'UTC')).toEqual(
        '2003-09-10 03-02-04+0000'
      );
    });
