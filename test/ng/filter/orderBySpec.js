describe('Filter: orderBy', function () {
  ...
  describe('(Arrays)', function () {
    ...
    it('should compare timestamps when sorting dates', function () {
      expect(orderBy([new Date('01/01/2015'), new Date('01/01/2014')])).toEqualData([
        new Date('01/01/2014'),
        new Date('01/01/2015')
      ]);
    });
    ...
    it('should support string predicates with names containing non-identifier characters', function () {
      ...
      expect(orderBy([{ 원: 76000 }, { 원: 31000 }, { 원: 156000 }], '"원"')).toEqualData([
        { 원: 31000 },
        { 원: 76000 },
        { 원: 156000 }
      ]);
    ...
    it('should sort nulls as Array.prototype.sort', function () {
      var array = [{ id: 2 }, null, { id: 3 }, null];
      expect(orderBy(array)).toEqualData([{ id: 2 }, { id: 3 }, null, null]);
    });
    ...
    it('should sort mixed array of objects and values in a stable way', function () {
      expect(orderBy([{ foo: 2 }, { foo: {} }, { foo: 3 }, { foo: 4 }], 'foo')).toEqualData([
        { foo: 2 },
        { foo: 3 },
        { foo: 4 },
        { foo: {} }
    ...
    describe('(reversing order)', function () {
      it('should not reverse collection if `reverse` param is falsy', function () {
        var items = [{ a: 2 }, { a: 15 }];
        var expr = 'a';
        var sorted = [{ a: 2 }, { a: 15 }];
      ...
      it('should reverse collection if `reverse` param is truthy', function () {
        var items = [{ a: 2 }, { a: 15 }];
        var expr = 'a';
        var sorted = [{ a: 15 }, { a: 2 }];
      ...
      it('should reverse collection if `reverse` param is `true`, even without an `expression`', function () {
    ...
    describe('(custom comparator)', function () {
      ...
      it('should use `index` as `value` if no other predicate can distinguish between two items', function () {
        var items = ['foo', 'bar'];
        var expr = null;
        var reverse = null;
    ...
    describe('(object as `value`)', function () {
      ...
      it('should use the return value of `valueOf()` for subsequent steps (if non-primitive)', function () {
        var o1 = {
          k: 1,
          valueOf: function () {
            return o3;
      ...
      it('should use the return value of `toString()` for subsequent steps (if non-primitive)', function () {
        var o1 = {
          k: 1,
          toString: function () {
            return o3;
