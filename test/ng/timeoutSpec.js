describe('$timeout', function () {
  ...
  it('should NOT call $evalAsync or $digest if invokeApply is set to false', inject(function ($timeout, $rootScope) {
  ...
  it('should return a promise which will be resolved with return value of the timeout callback', inject(function (
    $timeout,
    log
  ) {
  ...
  it('should forget references to deferreds when callback called even if skipApply is true', inject(function (
    $timeout,
    $browser
  ) {
  ...
  it('should pass the timeout arguments in the timeout callback', inject(function ($timeout, $browser, log) {
    ...
    $timeout(9000, false, 'What!', 9000).then(function (value) {
  ...
  describe('exception handling', function () {
    ...
    it('should delegate exception to the $exceptionHandler service', inject(function ($timeout, $exceptionHandler) {
      $timeout(function () {
        throw 'Test Error';
      });
    ...
    it('should call $apply even if an exception is thrown in callback', inject(function ($timeout, $rootScope) {
      var applySpy = spyOn($rootScope, '$apply').and.callThrough();
    ...
    it('should reject the timeout promise when an exception is thrown in the timeout callback', inject(function (
      $timeout,
      log
    ) {
      var promise = $timeout(function () {
      ...
      promise.then(log.fn('success'), function (reason) {
        log('error: ' + reason);
      });
      $timeout.flush();
    ...
    it('should pass the timeout arguments in the timeout callback even if an exception is thrown', inject(function (
      $timeout,
      log
    ) {
      var promise1 = $timeout(
        function (arg) {
          throw arg;
        },
        9000,
      ...
      promise1.then(log.fn('success'), function (reason) {
        log('error: ' + reason);
      });
      promise2.then(log.fn('success'), function (reason) {
        log('error: ' + reason);
      });
    ...
    it('should forget references to relevant deferred even when exception is thrown', inject(function (
      $timeout,
      $browser
    ) {
      // $browser.defer.cancel is only called on cancel if the deferred object is still referenced
