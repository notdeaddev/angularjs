...
describe('ngProp*', function () {
  ...
  it('should use the full ng-prop-* attribute name in $attr mappings', function () {
    ...
    inject(function ($compile, $rootScope) {
      $compile('<div attr-exposer ng-prop-title="12" ng-prop-super-title="34" ng-prop-my-camel_title="56">')(
        $rootScope
      );
  ...
  it('should disallow property binding to onclick', inject(function ($compile, $rootScope) {
    ...
    }).toThrowMinErr('$compile', 'nodomevents', 'Property bindings for HTML DOM event properties are disallowed');
    expect(function () {
      $compile('<button ng-prop-ONCLICK="onClickJs"></button>');
    }).toThrowMinErr('$compile', 'nodomevents', 'Property bindings for HTML DOM event properties are disallowed');
  }));
    ...
    if (msie !== 9 || srcsetElement === 'img') {
      describe(srcsetElement + '[srcset] sanitization', function () {
        it('should not error if srcset is blank', inject(function ($compile, $rootScope) {
          var element = $compile('<' + srcsetElement + ' ng-prop-srcset="testUrl"></' + srcsetElement + '>')(
            $rootScope
          );
        ...
        it('should NOT work with trusted values', inject(function ($rootScope, $compile, $sce) {
          ...
          element = $compile(
            '<' + srcsetElement + ' ng-prop-srcset="testUrl + \',\' + testUrl"></' + srcsetElement + '>'
          )($rootScope);
          $rootScope.testUrl = $sce.trustAsUrl('javascript:something');
          $rootScope.$digest();
          expect(element.prop('srcset')).toEqual('unsafe:javascript:something ,unsafe:javascript:something');
        }));
        ...
        it('should use $$sanitizeUri', function () {
          ...
          inject(function ($compile, $rootScope) {
            ...
            element = $compile('<' + srcsetElement + ' ng-prop-srcset="\'java\' + testUrl"></' + srcsetElement + '>')(
              $rootScope
            );
  ...
  describe('*[innerHTML]', function () {
    ...
    describe('SCE enabled', function () {
      ...
      it('should not cause infinite recursion for trustAsHtml object watches', inject(function (
        $rootScope,
        $compile,
        $sce
      ) {
