...
describe('Scope', function () {
  ...
  describe('$watch/$digest', function () {
    ...
    describe('constants cleanup', function () {
      ...
      it('should remove $watchCollection of filtered constant literals after initial digest', inject(function (
        $rootScope
      ) {
    ...
    it('should call child $watchers in addition order', inject(function ($rootScope) {
      ...
      childA.$watch('a', function () {
        log += 'a';
      });
      childB.$watch('b', function () {
        log += 'b';
      });
      childC.$watch('c', function () {
        log += 'c';
      });
    ...
    it('should allow $digest on a child scope with and without a right sibling', inject(function ($rootScope) {
      ...
      childA.$watch(function () {
        log += 'a';
      });
      childB.$watch(function () {
        log += 'b';
      });
    ...
    it('should prevent infinite recursion and print watcher function name or body', inject(function ($rootScope) {
      $rootScope.$watch(
        function watcherA() {
          return $rootScope.a;
        },
    ...
    it('should not skip watchers when adding new watchers during digest', inject(function ($rootScope) {
      var log = [];
    ...
    it('should not run the current watcher twice when removing a watcher during digest', inject(function ($rootScope) {
      var log = [];
      var removeWatcher3;
    ...
    it('should not skip watchers when removing itself during digest', inject(function ($rootScope) {
      var log = [];
      var removeWatcher1;
    ...
    it('should always call the watcher with newVal and oldVal equal on the first run', inject(function ($rootScope) {
      var log = [];
    ...
    describe('$watch deregistration', function () {
      ...
      it('should not mess up the digest loop if deregistration happens during digest', inject(function (
        $rootScope,
        log
      ) {
        ...
        var deregWatch1 = scope.$watch(log.fn('watch1'), function () {
          deregWatch1();
          log('watchAction1');
        });
        ...
        scope = $rootScope.$new();
        scope.$watch(log.fn('watch1'), function () {
          deregWatch2();
          log('watchAction1');
        });
        ...
        expect(log).toEqual(['watch1', 'watchAction1', 'watch3', 'watchAction3', 'watch1', 'watch3']);
        scope.$destroy();
        log.reset();
        ...
        scope.$watch(log.fn('watch2'), function () {
          deregWatch1();
          log('watchAction2');
        });
        scope.$watch(log.fn('watch3'), log.fn('watchAction3'));
    ...
    describe('$watchCollection', function () {
      describe('variable', function () {
        ...
        describe('array', function () {
          it('should return oldCollection === newCollection only on the first listener call', inject(function (
            $rootScope,
            log
          ) {
        ...
        describe('object', function () {
          ...
          it('should watch object properties', function () {
            ...
            $rootScope.$digest();
            expect(log.empty()).toEqual([{ newVal: { a: [], b: [], c: 'B' }, oldVal: { a: 'B', b: [], c: {} } }]);
      ...
      describe('literal', function () {
        ...
        describe('object', function () {
          ...
          it('should not trigger change when object in collection changes', function () {
            ...
            $rootScope.$digest();
            expect(log.empty()).toEqual([
              { newVal: { a: { name: 'foo' } }, oldVal: { a: { name: 'foo' } }, identical: true }
            ]);
        ...
        describe('object computed property', function () {
          ...
          it('should default to "undefined" key', function () {
            ...
            $rootScope.$digest();
            expect(log.empty()).toEqual([
              { newVal: { undefined: 'test' }, oldVal: { undefined: 'test' }, identical: true }
            ]);
          });
    ...
    describe('$suspend/$resume/$isSuspended', function () {
      ...
      it('should resume digesting immediately if `$resume` is called from an ancestor scope watch handler', inject(function (
        $rootScope
      ) {
        var watchSpy = jasmine.createSpy('watchSpy');
        var scope = $rootScope.$new();
      ...
      it('should resume digesting immediately if `$resume` is called from a non-ancestor scope watch handler', inject(function (
        $rootScope
      ) {
        var watchSpy = jasmine.createSpy('watchSpy');
        var scope = $rootScope.$new();
      ...
      it('should return false from `$isSuspended()` for a non-suspended scope that has a suspended ancestor', inject(function (
        $rootScope
      ) {
    ...
    describe('optimizations', function () {
      ...
      it('should quit digest early after we check the last watch that was previously dirty', inject(function (
        log,
        $rootScope
      ) {
        setupWatches($rootScope, log);
      ...
      it('should not quit digest early if a new watch was added from an existing watch action', inject(function (
        log,
        $rootScope
      ) {
        setupWatches($rootScope, log);
        ...
        $rootScope.$digest();
        expect(log).toEqual(['w1', 'w2', 'w3', 'w4', 'w4action', 'w5', 'w5action', 'w1', 'w2', 'w3', 'w4', 'w5']);
      }));

      it('should not quit digest early if an evalAsync task was scheduled from a watch action', inject(function (
        log,
        $rootScope
      ) {
        setupWatches($rootScope, log);
        ...
        $rootScope.$digest();
        expect(log).toEqual(['w1', 'w2', 'w3', 'w4', 'w4action', 'evalAsync', 'w1', 'w2', 'w3', 'w4']);
      }));
  ...
  describe('$destroy', function () {
    ...
    it('should $destroy a scope only once and ignore any further destroy calls', inject(function ($rootScope) {
      $rootScope.$digest();
      expect(log).toBe('123');
    ...
    it("should do nothing when a child event listener is registered after parent's destruction", inject(function (
      $rootScope
    ) {
      var parent = $rootScope.$new(),
        child = parent.$new();
    ...
    it("should do nothing when a child watch is registered after parent's destruction", inject(function ($rootScope) {
      var parent = $rootScope.$new(),
        child = parent.$new();
    ...
    it('should preserve all (own and inherited) model properties on a destroyed scope', inject(function ($rootScope) {
  ...
  describe('$evalAsync', function () {
    ...
    it('should operate only with a single queue across all child and isolate scopes', inject(function (
      $rootScope,
      $parse
    ) {
      var childScope = $rootScope.$new();
  ...
  describe('$apply', function () {
    ...
    describe('recursive $apply protection', function () {
      it('should throw an exception if $apply is called while an $apply is in progress', inject(function ($rootScope) {
        expect(function () {
          $rootScope.$apply(function () {
            $rootScope.$apply();
          });
      ...
      it('should not clear the state when calling $apply during an $apply', inject(function ($rootScope) {
        $rootScope.$apply(function () {
          expect(function () {
            $rootScope.$apply();
          }).toThrowMinErr('$rootScope', 'inprog', '$apply already in progress');
      ...
      it('should throw an exception if $apply is called while flushing evalAsync queue', inject(function ($rootScope) {
        expect(function () {
          $rootScope.$apply(function () {
            $rootScope.$evalAsync(function () {
              $rootScope.$apply();
      ...
      it('should throw an exception if $apply is called while a watch is being initialized', inject(function (
        $rootScope
      ) {
      ...
      it('should thrown an exception if $apply in called from a watch fn (after init)', inject(function ($rootScope) {
  ...
  describe('$applyAsync', function () {
    ...
    it('should pass thrown exceptions to $exceptionHandler', inject(function ($rootScope, $browser, $exceptionHandler) {
      ...
      $browser.defer.flush();
      expect($exceptionHandler.errors).toEqual(['OOPS']);
    }));
    ...
    it('should be cancelled if a $rootScope digest occurs before the next tick', inject(function (
      $rootScope,
      $browser
    ) {
  ...
  describe('$$postDigest', function () {
    ...
    it('should run a $$postDigest call on all child scopes when a parent scope is digested', inject(function (
      $rootScope
    ) {
      var parent = $rootScope.$new(),
        child = parent.$new(),
  ...
  describe('events', function () {
    describe('$on', function () {
      ...
      describe('deregistration', function () {
        ...
        it('should call next listener after removing the current listener via its own handler', inject(function (
          $rootScope
        ) {
          var listener1 = jasmine.createSpy('listener1').and.callFake(function () {
            remove1();
          });
        ...
        it('should call all subsequent listeners when a previous listener is removed via a handler', inject(function (
          $rootScope
        ) {
    ...
    describe('$emit', function () {
      ...
      it('should allow all events on the same scope to run even if stopPropagation is called', function () {
        child.$on('myEvent', logger);
        grandChild.$on('myEvent', function (e) {
          e.stopPropagation();
        });
        grandChild.$on('myEvent', logger);
        grandChild.$on('myEvent', logger);
      ...
      it('should dispatch exceptions to the $exceptionHandler', inject(function ($exceptionHandler) {
        child.$on('myEvent', function () {
    ...
    describe('$broadcast', function () {
      describe('event propagation', function () {
        var log, child1, child2, child3, grandChild11, grandChild21, grandChild22, grandChild23, greatGrandChild211;
        ...
        it('should not descend past scopes with a $$listerCount of 0 or undefined', inject(function ($rootScope) {
          var EVENT = 'fooEvent',
            spy = jasmine.createSpy('listener');
      ...
      describe('listener', function () {
        ...
        it("should have the event's `currentScope` property set to null after broadcast", inject(function ($rootScope) {
          var scope = $rootScope,
            child = scope.$new(),
            event;
