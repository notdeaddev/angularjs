...
describe('$interpolate', function () {
  ...
  it('should return undefined when there are no bindings and textOnly is set to true', inject(function ($interpolate) {
    expect($interpolate('some text', true)).toBeUndefined();
  }));

  it('should return undefined when there are bindings and strict is set to true', inject(function ($interpolate) {
  ...
  describe('interpolation escaping', function () {
    ...
    it('should unescape multiple expressions', inject(function ($interpolate) {
      ...
      expect($interpolate('{{foo}}\\{\\{foo\\}\\}{{bar}}\\{\\{bar\\}\\}{{foo}}')(obj)).toBe(
        'Hello{{foo}}World{{bar}}Hello'
      );
    }));
  ...
  describe('isTrustedContext', function () {
    it('should NOT interpolate a multi-part expression when isTrustedContext is RESOURCE_URL', inject(function (
      $sce,
      $interpolate
    ) {
    ...
    it('should interpolate and sanitize a multi-part expression when isTrustedContext is URL', inject(function (
      $sce,
      $interpolate
    ) {
      expect($interpolate('some/{{id}}', true, $sce.URL)({})).toEqual('some/');
      expect($interpolate('some/{{id}}', true, $sce.URL)({ id: 'javascript:' })).toEqual('some/javascript:');
      expect($interpolate('{{foo}}{{bar}}', true, $sce.URL)({ foo: 'javascript:', bar: 'javascript:' })).toEqual(
        'unsafe:javascript:javascript:'
      );
    }));
  });
  ...
  describe('startSymbol', function () {
    ...
    it(
      'should expose the startSymbol in config phase',
      module(function ($interpolateProvider) {
        expect($interpolateProvider.startSymbol()).toBe('((');
      })
    );
  ...
  describe('endSymbol', function () {
    ...
    it(
      'should expose the endSymbol in config phase',
      module(function ($interpolateProvider) {
        expect($interpolateProvider.endSymbol()).toBe('))');
      })
    );
