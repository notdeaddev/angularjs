...
describe('$log', function () {
  ...
  it('should use console if present', inject(
    function () {
      $window.console = { log: log, warn: warn, info: info, error: error, debug: debug };
    },
    function ($log) {
      $log.log();
      $log.warn();
  ...
  it('should use noop if no console', inject(function ($log) {
    $log.log();
    $log.warn();
    $log.info();
    $log.error();
  ...
  function runTests(options) {
    ...
    describe(ie9Mode ? 'IE 9 logging behavior' : "Modern browsers' logging behavior", function () {
      ...
      if (
        !/\b9\.\d(\.\d+)* safari/i.test(window.navigator.userAgent) &&
