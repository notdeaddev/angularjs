...
describe('parser', function () {
  ...
  describe('ast', function () {
    ...
    beforeEach(function () {
      ...
      createAst = function () {
        ...
        var ast = new AST(lexer, {
          csp: false,
          literals: { true: true, false: false, undefined: undefined, null: null }
        });
    ...
    it('should understand literals', function () {
      ...
      forEach(
        { 123: 123, '"123"': '123', true: true, false: false, null: null, undefined: undefined },
  ...
  forEach([true, false], function (cspEnabled) {
    ...
    it(
      'should allow extending literals with csp ' + cspEnabled,
  ...
  forEach([true, false], function (cspEnabled) {
    describe('csp: ' + cspEnabled, function () {
      beforeEach(
        module(function () {
          expect(csp().noUnsafeEval === true || csp().noUnsafeEval === false).toEqual(true);
          csp().noUnsafeEval = cspEnabled;
        }, provideLog)
      );
      ...
      it('should parse ternary', function () {
        ...
        // Function calls.
        expect(scope.$eval('returnTrue() ? returnString() : returnInt()')).toEqual(
          returnTrue() ? returnString() : returnInt()
        );
        expect(scope.$eval('returnFalse() ? returnString() : returnInt()')).toEqual(
          returnFalse() ? returnString() : returnInt()
        );
        expect(scope.$eval('returnTrue() ? returnString() : returnInt()')).toEqual(
          returnTrue() ? returnString() : returnInt()
        );
        expect(scope.$eval('identity(returnFalse() ? returnString() : returnInt())')).toEqual(
      ...
      it('should parse filters', function () {
        $filterProvider.register(
          'substring',
          valueFn(function (input, start, end) {
      ...
      it('should throw syntax error exception for identifiers ending with a dot', function () {
        ...
        expect(function () {
          scope.$eval('a.', scope);
        }).toThrowMinErr('$parse', 'ueoe', 'Unexpected end of expression: a.');
      ...
      it('should evaluate object', function () {
        ...
        expect(scope.$eval('{x, 1: x, [x = x + 1]: x, 3: x + 1, [x = x + 2]: x, 5: x + 1}', { x: 1 })).toEqual({
          x: 1,
          1: 1,
          2: 2,
          3: 3,
      ...
      describe('assignable', function () {
        ...
        it('should expose working assignment function for expressions with brackets in the middle', inject(function (
          $parse
        ) {
          var fn = $parse('a["b"].c');
          expect(fn.assign).toBeTruthy();
      ...
      describe('one-time binding', function () {
        ...
        it('should have a stable value if at the end of a $digest it has a defined value', inject(function (
          $parse,
          $rootScope,
          log
        ) {
        ...
        it('should invoke a stateless filter once when the parsed expression has an interceptor', inject(function (
          $parse,
          $rootScope
        ) {
        ...
        describe('literal expressions', function () {
          ...
          [true, false].forEach(function (isDeep) {
            describe(isDeep ? 'deepWatch' : 'watch', function () {
              it('should only become stable when all the properties of an object have defined values', inject(function (
                $parse,
                $rootScope,
                log
              ) {
              ...
              it('should only become stable when all the elements of an array have defined values', inject(function (
                $parse,
                $rootScope,
                log
              ) {
              ...
              it('should only become stable when all the elements of an array have defined values at the end of a $digest', inject(function (
                $parse,
                $rootScope,
                log
              ) {
      ...
      describe('watched $parse expressions', function () {
        ...
        describe('filters', function () {
          ...
          it('should ignore changes within nested objects (one-time)', function () {
            var watchCalls = [];
            scope.$watch('::[a, undefined]', function (a) {
              watchCalls.push(a[0]);
            });
            scope.a = 0;
            scope.$digest();
          ...
          describe('with non-primitive input', function () {
            describe('that does NOT support valueOf()', function () {
              ...
              it('should always be reevaluated in literals', inject(function ($parse) {
                ...
                expect(function () {
                  scope.$apply('a = {b: 1}');
                }).toThrowMinErr('$rootScope', 'infdig');
              }));
            ...
            describe('that does support valueOf()', function () {
              ...
              it('should not be reevaluated when the instance changes but valueOf() does not', inject(function (
                $parse
              ) {
                var filterCalls = 0;
                $filterProvider.register(
            ...
            it('should not be reevaluated when input is simplified via non-plus/concat binary operators', inject(function (
              $parse
            ) {
              var filterCalls = 0;
              $filterProvider.register(
        ...
        describe('interceptorFns', function () {
          ...
          it('should only be passed the intercepted value when double-intercepted', inject(function ($parse) {
            var args1;
            function int1(v) {
          ...
          it('should always be invoked if they are flagged as having $stateful', inject(function ($parse) {
            var called = false;
          ...
          it('should always be invoked if flagged as $stateful when wrapping one-time', inject(function ($parse) {
          ...
          it('should always be invoked if flagged as $stateful when wrapping one-time with inputs', inject(function (
            $parse
          ) {
          ...
          it('should always be invoked if flagged as $stateful when wrapping one-time literal', inject(function (
            $parse
          ) {
            var interceptorCalls = 0;
            function interceptor() {
          ...
          it('should not be invoked unless the input.valueOf() changes even if the instance changes', inject(function (
            $parse
          ) {
            var called = false;
            function interceptor(v) {
          ...
          it('should not affect when a one-time binding becomes stable', inject(function ($parse) {
            ...
            ); //interceptor that returns non-undefined
          ...
          it('should not affect when a one-time literal binding becomes stable', inject(function ($parse) {
            ...
            ); //interceptor that returns non-literal
          ...
          it('should watch the intercepted value of one-time bindings in nested interceptors', inject(function (
            $parse,
            log
          ) {
            scope.$watch(
        ...
        describe('literals', function () {
          ...
          describe('with non-primative input', function () {
            ...
            describe('that does support valueOf()', function () {
              ...
              it('should be reevaluated when the instance does not change but valueOf() does', inject(function (
                $parse
              ) {
                scope.date = new Date(1234567890123);
        ...
        it('should continue with the evaluation of the expression without invoking computed parts', inject(function (
          $parse
        ) {
          var value = 'foo';
      ...
      describe('locals', function () {
        ...
        they('should not overwrite $prop scope properties when assigning', [0, false, '', NaN], function (falsyValue) {
      ...
      describe('constant', function () {
        ...
        it('should not mark any expression involving variables or function calls as constant', inject(function (
          $parse
        ) {
      ...
      describe('null/undefined in expressions', function () {
        ...
        it('should return undefined if the return value of a function invocation is undefined', inject(function (
          $rootScope
        ) {
        ...
        it('should ignore undefined values when doing addition/concatenation', inject(function ($rootScope) {
          $rootScope.fn = function () {};
  ...
  forEach([true, false], function (cspEnabled) {
    describe('custom identifiers (csp: ' + cspEnabled + ')', function () {
      ...
      beforeEach(
        module(function ($parseProvider) {
          isIdentifierStartFn = jasmine.createSpy('isIdentifierStart').and.callFake(function (ch, cp) {
            return isIdentifierStartRe.test(ch);
          });
          isIdentifierContinueFn = jasmine.createSpy('isIdentifierContinue').and.callFake(function (ch, cp) {
            return isIdentifierContinueRe.test(ch);
          });
