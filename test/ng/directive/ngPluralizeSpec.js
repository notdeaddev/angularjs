...
describe('ngPluralize', function () {
  ...
  describe('deal with pluralized strings without offset', function () {
    ...
    it('should show single/plural strings with mal-formed inputs', inject(function ($log, $rootScope) {
      $rootScope.email = '';
      $rootScope.$digest();
      ...
      expect($log.debug.logs.shift()).toEqual(["ngPluralize: no rule defined for 'NaN' in undefined"]);
  ...
  describe('edge cases', function () {
    ...
    it('should be able to specify a message for null/undefined values', inject(function ($compile, $rootScope) {
      element = $compile(
        '<ng:pluralize count="email"' +
  ...
  describe('undefined rule cases', function () {
    ...
    it('should generate a warning when being asked to use a rule that is not defined', inject(function (
      $rootScope,
      $compile
    ) {
      element = $compile(
      ...
      expect(element.text()).toBe('');
      expect($log.debug.logs.shift()).toEqual([
        "ngPluralize: no rule defined for 'few' in {'0': 'Zero','one': 'Some text','other': 'Some text'}"
      ]);
    }));

    it('should empty the element content when using a rule that is not defined', inject(function (
      $rootScope,
      $compile
    ) {
      element = $compile(
      ...
      $locale.pluralCat = function (count) {
  ...
  describe('bind-once', function () {
    it('should support for `count` to be a one-time expression', inject(function ($compile, $rootScope) {
      element = $compile(
        '<ng:pluralize count="::email"' +
          "when=\"{'one': 'You have one new email'," +
          "'other': 'You have {} new emails'}\">" +
    ...
    it('should still update other embedded expressions', inject(function ($compile, $rootScope) {
      element = $compile(
        '<ng:pluralize count="::email"' +
