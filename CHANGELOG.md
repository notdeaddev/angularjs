...
1. Passing an array or jqLite/jQuery collection of parent-less text nodes to `$compile` directly:
   ...
   ```js
   // Assuming:
   var textNodes = [document.createTextNode('{{'), document.createTextNode('"foo:"'), document.createTextNode('}}')];
   ...
   var textNodes = [document.createTextNode('{{'), document.createTextNode('"foo"'), document.createTextNode('}}')];
   var compiledNodes = angular.element(
...
2. Using multi-slot transclusion with non-consecutive, default-content text nodes (that form
   ...
   <!-- After: -->
   <some-thing>
     <ng-transclude>foo <-- The text nodes were merged into `{{'foo'}}`, which was then interpolated</ng-transclude>
   </some-thing>
...
```html
<div ng-click="onClick()" ng-keypress="onClick()">I respond to `click` and `keypress` (not `keydown`)</div>
<!-- OR -->
<div ng-click="onClick()">I respond to `click` and `keydown` (not `keypress`)</div>
```
...
describe('$q.when', function () {
  it('should not need a call to $timeout.flush() to resolve already resolved promises', inject(function ($q, $timeout) {
    $q.when('foo');
...
```js
angular.module('switchChangeWorkaround', []).directive('onSwitchChanged', function () {
  return {
