...
module.exports = {
  ...
  min: function (file, done) {
    ...
    var compilationLevel =
      file === 'build/angular-message-format.js' ? 'ADVANCED_OPTIMIZATIONS' : 'SIMPLE_OPTIMIZATIONS';
    shell.exec(
      'java ' +
        this.java32flags() +
        ' ' +
        ...
        ' ' +
        '-cp vendor/closure-compiler/compiler.jar' +
        classPathSep +
      ...
      function (code) {
        ...
        grunt.file.write(
          mapFile,
          grunt.file.read(mapFile).replace('"file":"build/', '"file":"').replace('"sources":["build/', '"sources":["')
        );
