...
var process = require('process'); // eslint-disable-line no-redeclare
...
var getCodeName = function (tagName) {
  ...
  var tagMessage = gitCatOutput.match(/^.*codename.*$/gm)[0];
  var codeName = tagMessage && tagMessage.match(/codename\((.*)\)/)[1];
  if (!codeName) {
    throw new Error(
      'Could not extract release code name. The message of tag ' +
        tagName +
  ...
  if (tagResults.code === 0) {
    return _(tagResults.stdout.match(/v[0-9].*[0-9]$/gm))
      ...
      .map(function (version) {
        ...
        if (
          (version.major === 1 && version.minor === 0 && version.prerelease.length > 0) ||
          (version.major === 1 && version.minor === 2 && version.prerelease[0] === 'rc1')
        ) {
    ...
    .reduce(function (cdnVersion, version) {
      if (!cdnVersion) {
        if (NO_REMOTE_REQUESTS) {
          ...
          var cdnResult = shell.exec(
            'curl http://ajax.googleapis.com/ajax/libs/angularjs/' +
              version +
              '/angular.min.js ' +
